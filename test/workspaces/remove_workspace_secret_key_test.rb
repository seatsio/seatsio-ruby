...
class AddWorkspaceSecretKeyTest < SeatsioTestClient
  def test_add_secret_key
    ...
    @seatsio.workspaces.remove_secret_key key: workspace.key, secret_key_to_remove: workspace.secret_key

    final_state_workspace = @seatsio.workspaces.retrieve key: workspace.key
    assert_equal([new_secret_key], final_state_workspace.secret_keys)
  end
end
