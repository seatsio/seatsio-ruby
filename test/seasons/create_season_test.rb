...
class CreateSeasonTest < SeatsioTestClient
  ...
  def test_channels_can_be_passed_in
    chart_key = create_test_chart
    season = @seatsio.seasons.create chart_key: chart_key, channels: [
    ...
    expected_channels = [
      Seatsio::Channel.new("channelKey1", season.channels[0].id, "channel 1", "#FF0000", 1, %w[A-1 A-2], {}),
      Seatsio::Channel.new("channelKey2", season.channels[1].id, "channel 2", "#0000FF", 2, [], {})
    ]
    assert_equal(expected_channels, season.channels)
  end
