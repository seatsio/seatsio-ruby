...
class WorkspaceKeyAuthenticationTest < SeatsioTestClient
  def test_client_takes_optional_workspace_key
    ...
    subaccount_client = Seatsio::Client.new(@user['secretKey'], subaccount.workspace.key, "https://api-staging.seatsio.net")
    hold_token = subaccount_client.hold_tokens.create

    assert_equal(subaccount.workspace.key, hold_token.workspace_key)
  end
end
